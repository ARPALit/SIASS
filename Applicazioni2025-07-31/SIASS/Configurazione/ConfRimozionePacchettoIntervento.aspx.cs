...
namespace SIASS
{
    public partial class ConfRimozionePacchettoIntervento : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
			...
			if (!Page.IsPostBack)
            {
                using (SIASSEntities context = new SIASSEntities())
                {
                    ...
                    ElencoTipiPacchettoDropDownList.DataSource = ConfigurazioneManager.ElencoTipiPacchettoInUsoDaStrumentiLetturaSulCampo();
        ...
        {
            ConfermaLabel.Text = $"Rimuovere {ElencoTipiPacchettoDropDownList.SelectedItem.Text} dalle {StazioniGridView.Rows.Count} stazioni selezionate?";
            ConfigurazioneMultiView.SetActiveView(ConfermaView);
        }

        protected void ConfermaButton_Click(object sender, EventArgs e)
        {
            logger.Debug($"Rimozione pacchetto {ElencoTipiPacchettoDropDownList.SelectedValue}");
            int numeroStazioniSelezionate = StazioniGridView.Rows.Count;
            int pacchettiRimossi = 0;
            int pacchettiNonRimossi = 0;
            for (int i = 0; i < numeroStazioniSelezionate; i++)
            {
                decimal idStazione = (decimal)StazioniGridView.DataKeys[i].Values["IdStazione"];
                if (ConfigurazioneManager.RimuoviPacchettoIntervento(
                    idStazione,
                    decimal.Parse(ElencoTipiPacchettoDropDownList.SelectedValue)
                    ))
                {
                    logger.Debug($"IdStazione {idStazione} pacchetto rimosso");
                    pacchettiRimossi++;
                }
                else
                {
                    logger.Debug($"IdStazione {idStazione} pacchetto non rimosso");
                    pacchettiNonRimossi++;
                }
            }
            EsitoLabel.Text = $"Operazione completata.<br />Stazioni elaborate: {numeroStazioniSelezionate}<br />Pacchetti non rimossi (usati da interventi): {pacchettiNonRimossi}";
