...
namespace SIASS
{
    public partial class ConfAggiuntaGrandezzaCampo : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                using (SIASSEntities context = new SIASSEntities())
                {
                    ...
                    ElencoTipiGrandezzaDropDownList.DataSource = ConfigurazioneManager.ElencoTipiGrandezza();
                    ...
                    ElencoTipiUnitaMisuraDropDownList.DataSource = ConfigurazioneManager.ElencoTipiUnitaMisura();
        ...
        protected void SelezioneStazioniButton_Click(object sender, EventArgs e)
        {
            ...
            if (ElencoTipiUnitaMisuraDropDownList.SelectedIndex == 0)
                return;
            if (string.IsNullOrEmpty(NumeroDecimaliTextBox.Text.Trim()) || !int.TryParse(NumeroDecimaliTextBox.Text.Trim(), out _))
                return;

            ConfigurazioneMultiView.SetActiveView(SelezioneStazioniView);
        }

        protected void AggiungiButton_Click(object sender, EventArgs e)
        {
            ConfermaLabel.Text = $"Aggiungere {ElencoTipiGrandezzaDropDownList.SelectedValue} ({ElencoTipiUnitaMisuraDropDownList.SelectedValue}) alle {StazioniGridView.Rows.Count} stazioni selezionate?";
            ConfigurazioneMultiView.SetActiveView(ConfermaView);
        }

        protected void ConfermaButton_Click(object sender, EventArgs e)
        {
            logger.Debug($"Aggiunta grandezza {ElencoTipiGrandezzaDropDownList.SelectedValue} tipo unita misura {ElencoTipiUnitaMisuraDropDownList.SelectedValue}");
            int numeroStazioniSelezionate = StazioniGridView.Rows.Count;
            int grandezzeStazioniAggiunte = 0;
            int grandezzeStazioniNonAggiunte = 0;
            for (int i = 0; i < numeroStazioniSelezionate; i++)
            {
                decimal idStazione = (decimal)StazioniGridView.DataKeys[i].Values["IdStazione"];
                if (ConfigurazioneManager.AggiungiGrandezzaStazione(
                    idStazione,
                    ElencoTipiGrandezzaDropDownList.SelectedValue,
                    ElencoTipiUnitaMisuraDropDownList.SelectedValue,
                    decimal.Parse(NumeroDecimaliTextBox.Text.Trim())
                    ))
                {
                    logger.Debug($"IdStazione {idStazione} grandezza aggiunta");
                    grandezzeStazioniAggiunte++;
                }
                else
                {
                    logger.Debug($"IdStazione {idStazione} grandezza non aggiunta");
                    grandezzeStazioniNonAggiunte++;
                }
            }
            EsitoLabel.Text = $"Operazione completata.<br />Stazioni elaborate: {numeroStazioniSelezionate}<br />Grandezze aggiunte: {grandezzeStazioniAggiunte}<br />Grandezze non aggiunte (già presenti): {grandezzeStazioniNonAggiunte}";
