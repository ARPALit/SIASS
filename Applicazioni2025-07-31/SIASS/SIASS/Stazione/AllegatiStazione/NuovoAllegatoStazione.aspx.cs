...
namespace SIASS
{
	public partial class NuovoAllegatoStazione : System.Web.UI.Page
	{
		...
		protected void Page_Load(object sender, EventArgs e)
		{
			...
			if (!Page.IsPostBack)
			{
				...
				NomeFileAllegatoFileUpload.Attributes["accept"] = ConfigurationManager.AppSettings["EstensioniAccettate"];
			}
		}
		...
		private void RitornaElencoAllegati()
		{
			...
			Response.Redirect($"ElencoAllegatiStazione.aspx?IdStazione={idStazione}");
		}

		protected void SalvaButton_Click(object sender, EventArgs e)
		{
			logger.Debug($"Caricamento allegato stazione");
			var a = new AllegatoStazione
			{
				IdStazione = decimal.Parse(ViewState["IdStazione"].ToString())
			};
			...
			{
				logger.Warn($"File non specificato");
			...
			{
				context.AllegatiStazione.Add(a);
				context.SaveChanges();

				// Carica l'oggetto info per avere i dati di stazione
				InfoAllegatoStazione infoAllegatoStazione = AllegatoStazioneManager.CaricaInfoAllegatoStazione(a.IdAllegatoStazione);

				// Salva il file creando eventualmente la cartella della stazione 
				string cartellaStazione = "~/File/Allegati/Stazione" + infoAllegatoStazione.IdStazione;
				if (!Directory.Exists(Server.MapPath(cartellaStazione)))
					Directory.CreateDirectory(Server.MapPath(cartellaStazione));

				NomeFileAllegatoFileUpload.SaveAs(Page.MapPath(cartellaStazione + "/" + infoAllegatoStazione.NomeFileAllegato));
				logger.Info(string.Format("Inserito allegato stazione - id:{0} - id stazione:{1} - Operatore:{2}",
					infoAllegatoStazione.IdAllegatoStazione, infoAllegatoStazione.IdStazione, $"{oper.Nome} {oper.Cognome}"));
