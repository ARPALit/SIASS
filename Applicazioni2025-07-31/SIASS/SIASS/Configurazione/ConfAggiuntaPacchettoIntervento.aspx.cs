...
namespace SIASS
{
    public partial class ConfAggiuntaPacchettoIntervento : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
			...
			if (!Page.IsPostBack)
            {
                using (SIASSEntities context = new SIASSEntities())
                {
                    ...
                    logger.Debug($"Caricamento elenco tipi pacchetto");
                    ElencoTipiPacchettoDropDownList.DataSource = ConfigurazioneManager.ElencoTipiPacchetto();
                    ElencoTipiPacchettoDropDownList.DataBind();
                    ElencoTipiPacchettoDropDownList.Items.Insert(0, string.Empty);
                ...
                ConfigurazioneMultiView.SetActiveView(SelezionePacchettoView);
            }
        }
        ...
        {
            if (ElencoTipiPacchettoDropDownList.SelectedIndex == 0)
                return;
        ...
        {
            ConfermaLabel.Text = $"Aggiungere {ElencoTipiPacchettoDropDownList.SelectedItem.Text} alle {StazioniGridView.Rows.Count} stazioni selezionate?";
            ConfigurazioneMultiView.SetActiveView(ConfermaView);
        }

        protected void ConfermaButton_Click(object sender, EventArgs e)
        {
            logger.Debug($"Aggiunta pacchetto {ElencoTipiPacchettoDropDownList.SelectedValue}");
            int numeroStazioniSelezionate = StazioniGridView.Rows.Count;
            int pacchettiAggiunti = 0;
            int pacchettiNonAggiunti = 0;
            for (int i = 0; i < numeroStazioniSelezionate; i++)
            {
                decimal idStazione = (decimal)StazioniGridView.DataKeys[i].Values["IdStazione"];
                if (ConfigurazioneManager.AggiungiPacchettoIntervento(
                    idStazione,
                    decimal.Parse(ElencoTipiPacchettoDropDownList.SelectedValue)
                    ))
                {
                    logger.Debug($"IdStazione {idStazione} pacchetto aggiunto");
                    pacchettiAggiunti++;
                }
                else
                {
                    logger.Debug($"IdStazione {idStazione} pacchetto non aggiunto");
                    pacchettiNonAggiunti++;
                }
            }
            EsitoLabel.Text = $"Operazione completata.<br />Stazioni elaborate: {numeroStazioniSelezionate}<br />Pacchetti aggiunti: {pacchettiAggiunti}<br />Pacchetti non aggiunti (già presenti): {pacchettiNonAggiunti}";
