...
namespace SIASS
{
    public partial class ElencoMisurazioniOrganoclorurati : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
			...
			if (!Page.IsPostBack)
            {
                ...
                var elencoDate = OrganocloruratiManager.ElencoDateMisurazioni(infoStazione.CodiceIdentificativoSito, infoStazione.CodiceIdentificativo);
                if (elencoDate.Count > 0)
                {
                    DateDropDownList.DataSource = elencoDate;
                    DateDropDownList.DataBind();
                    AvvisoLabel.Visible = false;
                }
                else
                {
                    MisurazioniPanel.Visible = false;
                }
        ...
        protected void VisualizzaButton_Click(object sender, EventArgs e)
        {
            ...
            InfoStazione infoStazione = StazioneManager.CaricaInfoStazione(idStazione);
            DateTime dataMisurazione = DateTime.Parse(DateDropDownList.SelectedValue);
            ElencoMisurazioniGridView.DataSource = OrganocloruratiManager.ElencoMisurazioni(dataMisurazione, infoStazione.CodiceIdentificativoSito, infoStazione.CodiceIdentificativo);
