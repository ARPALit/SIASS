	...
	public class ControlloTEST()
	{
		...
		public Risposta EsegueControllo(Richiesta richiesta)
		{
			...
			try
			{
				...
				logger.Debug($"Controllo 'TEST' per IdGrandezzaStazione:{richiesta.IdGrandezzaStazione} configurazioneJSON:{richiesta.ConfigurazioneJSON}");
				...
				var elencoMisurazioniAnomale = context.SIAS_MISURAZIONI
					...
					&&
					i.VALORE > configurazioneControllo.TemperaturaMassima.Value
					&&
					i.DATA_MISURAZIONE >= (DateTime.Now.AddDays(-configurazioneControllo.NumeroGiorni.Value))
					)
					.ToList();
		...
		private class ConfigurazioneControllo
		{
			...
			public int? TemperaturaMassima { get; set; }
		}
