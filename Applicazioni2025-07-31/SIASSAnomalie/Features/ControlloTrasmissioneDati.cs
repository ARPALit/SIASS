...
{
	public class ControlloTrasmissioneDati()
	{
		...
		public Risposta EsegueControllo(Richiesta richiesta)
		{
			...
			try
			{
				...
				logger.Debug($"Controllo 'Trasmissione dati' per idGrandezza:{richiesta.IdGrandezzaStazione} configurazioneJSON:{richiesta.ConfigurazioneJSON}");
				...
				var inizioIntervallo = dataCorrente.AddHours(-configurazioneControllo.OreDaUltimoDatoDisponibile.Value);

				var presenzaMisurazioniRecenti = context.SIAS_MISURAZIONI
					.Where(
				...
				if (presenzaMisurazioniRecenti == false)
				{
					...
					{
						risposta.Valore = ultimaMisurazione.VALORE;
		...
		{
			public int? OreDaUltimoDatoDisponibile { get; set; }
		}
